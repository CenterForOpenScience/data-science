...
projects_and_logs %>%
  ...
  select(days_create_lastlog, products) %>%
  mutate(days_create_lastlog = round(log2(as.numeric(days_create_lastlog)))) %>%
  group_by(products, days_create_lastlog) %>%
  summarize(percent = n()) %>%
  group_by(days_create_lastlog) %>%
  mutate(percent = percent/sum(percent)*100) %>%
  filter(days_create_lastlog >= -22) %>%
  ggplot(aes(x = days_create_lastlog, y = percent, color = products)) +
  geom_line() +
  scale_x_continuous('Log2 Days Between Last Log and Date Created',
                      breaks = seq(-22, 12, by = 2)) +
  labs(y = 'Percentage of projects by product')
