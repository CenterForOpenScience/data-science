...
quartile_data %>%
  ...
  ungroup() %>%
  pivot_wider(names_from = quartile, values_from = n) %>%
  mutate(First = round(100 * First/nrow(quartile_data %>% filter(log_type == 'content' & quartile  == 'First')),2),
         Second = round(100 * Second/nrow(quartile_data %>% filter(log_type == 'content' & quartile  == 'Second')),2),
         Third = round(100 * Third/nrow(quartile_data %>% filter(log_type == 'content' & quartile  == 'Third')),2),
         Fourth = round(100 * Fourth/nrow(quartile_data %>% filter(log_type == 'content' & quartile  == 'Fourth')),2)) %>%
  select(-log_type) %>%
  arrange(desc(First)) %>%
  gt()
