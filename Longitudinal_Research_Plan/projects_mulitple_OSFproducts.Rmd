...
projects_and_logs_detailed <- projects_by_product %>%
                                select(root_id, project_created, has_file, registered, is_public) %>%
...
non_forks <- projects_and_logs %>% 
  filter(is.na(is_fork) & !is.na(days_create_lastlog))

quartiles <- quantile(non_forks$days_create_lastlog)

quartile_data <- non_forks %>%
  mutate(quartile = case_when(days_create_files < quartiles[2] ~ 'First',
                              days_create_files >= quartiles[2] & days_create_files < quartiles[3] ~ 'Second',
                              days_create_files >= quartiles[3] & days_create_files < quartiles[4] ~ 'Third',
                              days_create_files >= quartiles[4] ~ 'Fourth'))
