...
projects_by_product <- node_data %>%
                            ...
                            summarize(nodes_in_project = n(),
                                      ...
                                      last_public = if(all(is.na(date_made_public))) NA else max(date_made_public, na.rm = TRUE),
...
first_none <- later_project %>% filter(is.na(first_file) & is.na(first_reg) & is.na(last_public)) # number of none projects at first level

file_first <- later_project %>% filter(!is.na(first_file) & ((first_file < last_public & first_file < first_reg) | (first_file < last_public & is.na(first_reg)) | (is.na(last_public) & first_file < first_reg) | (is.na(last_public) & is.na(first_reg))))

public_first <- later_project %>% filter(!is.na(last_public) & ((last_public < first_file & last_public < first_reg) | (last_public < first_file & is.na(first_reg)) | (is.na(first_file) & last_public < first_reg) | (is.na(first_file) & is.na(first_reg))))
  
registered_first <- later_project %>% 
                        filter(!is.na(first_reg) & ((first_reg < last_public & first_reg < first_file) | 
                                                              (first_reg < last_public & is.na(first_file)) | 
                                                              (is.na(last_public) & first_reg < first_file) | 
                                                              (is.na(last_public) & is.na(first_file))))


file_then_reg <- file_first %>%
                    filter(!is.na(first_reg) & ((first_reg < last_public) | is.na(last_public)))
                    
file_then_public <- file_first %>%
                      filter(!is.na(last_public) & ((last_public < first_reg) | is.na(first_reg)))
file_then_none <- file_first %>% filter(is.na(last_public) & is.na(first_reg))
...
reg_then_public <- registered_first %>%
                      filter(!is.na(last_public) & ((last_public < first_file) | is.na(first_file)))
reg_then_file <- registered_first %>%
                      filter(!is.na(first_file) & ((first_file < last_public) | is.na(last_public)))
reg_then_node <- registered_first %>% filter(is.na(last_public) & is.na(first_file))
