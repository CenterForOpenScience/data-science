...
projects_by_product <- node_data %>%
                            ...
                            summarize(nodes_in_project = n(),
                                      ...
                                      first_public = if(all(is.na(date_made_public))) NA else max(date_made_public, na.rm = TRUE),
...
For this analysis, I removed projects that were OSF4M projects.
...
```{r}
ggplot(project_timeline %>% filter(time_action2 > 1), aes(time_action2, color = action2)) +
  geom_freqpoly(binwidth = 200) +
  scale_x_continuous('Days Between Project Created and Next Action',
                     breaks = round(seq(0, 1460, by = 90),1)) +
