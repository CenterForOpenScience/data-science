...
projects_by_product <- node_data %>%
                            ...
                            group_by(root_id) %>%
                            mutate(has_file = case_when(any(has_files == TRUE) ~ 1, all(has_files == FALSE) ~ 0)) %>%
                            mutate(shared_file = case_when(any(public_sharing == TRUE) ~ 1, all(public_sharing == FALSE) ~ 0)) %>%
                            mutate(registered = case_when(any(registered == TRUE) ~ 1, all(registered == FALSE) ~ 0)) %>%
                            mutate(is_suppnode = case_when(any(preprint_suppnode == TRUE) ~ 1, all(preprint_suppnode == FALSE) ~ 0)) %>%
                            mutate(is_osf4m = case_when(any(osf4m == TRUE) ~ 1, all(osf4m == FALSE) ~ 0)) %>%
                            summarize(has_file = max(has_file), 
                                      shared_file = max(shared_file), 
                                      registered = max(registered), 
                                      project_created = min(created),
                                      osf4m = max(is_osf4m),
                                      suppnode = max(is_suppnode),
                                      first_reg = min(first_reg),
                                      first_file = min(first_file),
                                      first_public = min(date_made_public)) %>%
