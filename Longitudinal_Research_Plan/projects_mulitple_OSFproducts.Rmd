...
projects_and_logs_detailed <- projects_by_product %>%
                                select(root_id, project_created, has_file, registered, is_public, is_fork) %>%
...
common_metadata_logs <- quartile_data %>%
  ...
  ungroup() %>%
  pivot_wider(names_from = quartile, values_from = n) %>%
  mutate(First = round(100 * First/nrow(quartile_data %>% filter(log_type == 'metadata' & quartile  == 'First')),2),
         Second = round(100 * Second/nrow(quartile_data %>% filter(log_type == 'metadata' & quartile  == 'Second')),2),
         Third = round(100 * Third/nrow(quartile_data %>% filter(log_type == 'metadata' & quartile  == 'Third')),2),
         Fourth = round(100 * Fourth/nrow(quartile_data %>% filter(log_type == 'metadata' & quartile  == 'Fourth')),2)) %>%
  select(-log_type) %>%
  filter(First >= 3 | Second >= 3 | Third >= 3 | Fourth >= 3) %>%
  select(action)
