...
common_logs %>%
  filter(log_type == 'content' & products == 'Empty_Private') %>%
  group_by(content_log_cats, root_id) %>%
  slice(1L) %>%
  ungroup() %>%
  group_by(content_log_cats, quartile) %>%
  tally() %>%
  ungroup() %>%
  left_join(total_ns_content %>% filter(products == 'Empty_Private'), by = 'quartile') %>%
  mutate(percentages = round(100*n.x/n.y, 2)) %>%
  select(content_log_cats, quartile, percentages) %>%
  pivot_wider(names_from = quartile, values_from = percentages) %>%
  ...
  tab_header(title = 'Empty Private Projects %s by Quartile',
...
common_logs %>%
  filter(log_type == 'content' & products == 'Files_Private') %>%
  group_by(content_log_cats, root_id) %>%
  slice(1L) %>%
  ungroup() %>%
  group_by(content_log_cats, quartile) %>%
  tally() %>%
  ungroup() %>%
  left_join(total_ns_content %>% filter(products == 'Files_Private'), by = 'quartile') %>%
  mutate(percentages = round(100*n.x/n.y, 2)) %>%
  select(content_log_cats, quartile, percentages) %>%
  pivot_wider(names_from = quartile, values_from = percentages) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%
  ...
  tab_header(title = 'Files Private Projects %s by Quartile',
                 subtitle = 'For content logs') %>%
  cols_align(align = c('center'), columns = T) %>%
  data_color(
    columns = vars(First,
...
rbind(table_input_content('Empty_Private'),
