...
projects_by_product <- node_data %>%
                            ...
                            group_by(root_id) %>%
                            mutate(has_file = case_when(any(has_files == TRUE) ~ 1, TRUE ~ 0)) %>%
                            mutate(shared_file = case_when(any(public_sharing == TRUE) ~ 1, TRUE ~ 0)) %>%
                            mutate(registered = case_when(any(registered == TRUE) ~ 1, TRUE ~ 0)) %>%
