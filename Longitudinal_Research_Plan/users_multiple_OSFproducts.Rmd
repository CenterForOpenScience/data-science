...
```{r echo=FALSE}
user_products_table_data <- user_categorization %>%
                        ...
                        mutate(product = case_when(product == 'nodes' ~ 'OSF Node',
                                                   ...
                                                   product == 'privatestorage' ~ 'Only Private nodes with files',
                                                   product == 'publicstorage' ~ 'Public OSF nodes with files',
                                                   product == 'suppnode' ~ 'Supplemental preprint node',
                                                   product == 'registrations' ~ 'Registrations')) %>%
                        mutate(product = as_factor(product)) %>%
                        mutate(product = fct_relevel(product, "No nodes or preprints", "osf4m", "OSF Node", "Only Private nodes with files", "Public OSF nodes with files", "Registrations", "Preprints", "Supplemental preprint node")) %>%
                        arrange(product) %>%
