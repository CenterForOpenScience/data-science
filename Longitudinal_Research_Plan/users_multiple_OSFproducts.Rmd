...
table_data <- gather(aggregate_numbers) %>% 
                  ...
                  mutate(key = case_when(grepl('on_none', key) ~ 'On none of the project types',
                                         grepl('on_private_file_project', key) ~ 'Only Private projects with files',
                                         grepl('on_sharing_project', key) ~ 'Projects sharing > 0 files',
...
The number of these users who have uploaded quickfiles is: `r data %>% filter(action_types == 'No products') %>% summarize(quickfiles = sum(quickfile_nodes > 0))`
