 ...
 * The data used for the analyses below goes through 12/30/19
 * Empty supplemental nodes that resulted from NPD have been removed from the data
 * The analysis only include **Non-Deleted/Withdrawn** nodes and registrations and published preprints
 ...
 * 'Public Sharing' is measured by looking for *non-registration* nodes that are public and have a file in them or an add-on connected. It does **not** include osf4m or preprint supplemental nodes.

The SQL code used to generate the data underlying this report can be found [here](https://github.com/CenterForOpenScience/data-science/blob/master/SQL_queries/user_activities.sql) 
...
user_data <- read_csv('/Users/courtneysoderberg/Documents/data-science/Longitudinal_Research_Plan/user_categorization.csv', col_types = cols(merged_by_id = col_character()))

user_preprint_data <- read_csv('/Users/courtneysoderberg/Documents/data-science/Longitudinal_Research_Plan/user_preprint_usage.csv')

#turn into long form
user_preprint_data <- rbind(user_preprint_data %>% select(-c(preprints_contributed, contributor_provider)) %>% rename(preprints = preprints_created, provider = creator_provider) %>% mutate(type = 'created'), user_preprint_data %>% select(-c(preprints_created, creator_provider)) %>% rename(preprints = preprints_contributed, provider = contributor_provider) %>% mutate(type = 'contributor')) %>% distinct() %>% filter(!is.na(provider))
```
...
gt() %>%
tab_header(title = 'Users by Product Type') %>%
cols_merge(columns = vars(created, created_prc), pattern = "{1} ({2}%)") %>%
cols_merge(columns = vars(on, on_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns =  vars(user, user_prc), pattern = '{1} ({2}%)') %>%
...
tab_row_group(
  group = 'Exclusive Multiple Product Usage',
  rows = 9:12)
                        
```

## Disciplines of 3 product users

```{r}

##create table of all 3 product users by which preprint service they use

#get provider userage by id
provider_users <- user_preprint_data %>%
                    group_by(id, provider) %>%
                    tally() %>%
                    select(-n)

#get usage for only all 3 product users
user_categorization %>%
  distinct(id, .keep_all = TRUE) %>%
  filter(is_active == TRUE) %>%
  filter(public_sharing_contributor == 1 & preprint_contributor == 1 & registrations_contributor == 1) %>%
  select(id, public_sharing_creator, public_sharing_contributor, public_sharing_user, preprint_creator, preprint_contributor, preprint_user, registrations_creator, registrations_contributor, registrations_user) %>%
  left_join(provider_users, by = 'id') %>%
  group_by(provider) %>%
  tally() %>%
  mutate(total_n = nrow(user_categorization %>%
  distinct(id, .keep_all = TRUE) %>%
  filter(is_active == TRUE) %>%
  filter(public_sharing_contributor == 1 & preprint_contributor == 1 & registrations_contributor == 1) %>%
  select(id, public_sharing_creator, public_sharing_contributor, public_sharing_user, preprint_creator, preprint_contributor, preprint_user, registrations_creator, registrations_contributor, registrations_user))) %>%
  mutate(percent = round(100*n/total_n,2)) %>%
  select(-total_n) %>%
  ...
  rename(Provider = provider, 
         ...
         Percentage = percent) %>%
  gt() %>%
  tab_header(title = 'Preprint providers used by users of 3 OSF products')
```
...
```{r echo = FALSE, warning=FALSE}
 graph <- ggplot(user_categorization %>% distinct(id, .keep_all = TRUE) %>% filter(user_type == 'All three') %>% filter(is_active == 'TRUE'), aes(by_day_confirmed)) + 
 ...
 graph2 <- ggplot(user_categorization %>% distinct(id, .keep_all = TRUE) %>% filter(is_active == 'TRUE') %>% filter(user_type != 'Only private storage'), aes(by_day_confirmed, color = user_type)) + 
  geom_freqpoly(binwidth = 30) +
  xlab('When user was confirmed') +
  ylab('Number of users') +
  scale_x_date(breaks = date_breaks("3 month"), labels = date_format("%b %y"))
...
The majority of users only have creation actions within one type of product. Below is a table showing for each product, the percentage of users who have only created that product type. The high proportion of single product type creators maps onto the low conditional probabilities in the creator vs. creator table above.
...
cols_merge(columns = vars(meetings, meetings_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(OSF, osf_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(registries, registries_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(preprints, preprints_prc), pattern = '{1} ({2}%)') %>%
cols_label(product = 'Product Type',
     meetings = 'osf4m',
     OSF = 'OSF',
...
## Origin by preprint providers

```{r warning=FALSE, echo=FALSE, message=FALSE}
user_categorization %>%
  filter(is_active == TRUE & is_invited == FALSE) %>%
  filter(user_origin == 'preprints' & preprint_user == 1 & user_tag != 'preprints_metrics') %>%
  group_by(user_tag) %>%
  summarize(n = n(),
            none = sum(preprint_user == 0 & osf4m_user == 0 & osf_node_user == 0 & registrations_user == 0 & suppnode_user == 0),
            privatestorage_only = sum(preprint_user == 0 & osf4m_user == 0 & private_storage_user ==1 & public_sharing_user == 0 & registrations_user == 0 & suppnode_user == 0),
            osf4m = sum(osf4m_user == 1),
            preprint= sum(preprint_user == 1),
            suppnode = sum(suppnode_user == 1),
...
filter(n >= 75) %>%  ## only keep pp-services that have generated at least 30 users for readability
melt(variable.name = 'product') %>% 
dcast(product ~ user_tag) %>%
mutate(product = case_when(
                      product == 'n' ~ 'Total',                   
                      product == 'nodes' ~ 'OSF Node',
                     product == 'none' ~ 'No nodes or preprints',
                     product == 'osf4m' ~ 'osf4m',
                     product == 'preprint' ~ 'Preprints',
                     product == 'privatestorage_only' ~ 'Only private storage',
                     ...
                     product == 'allthree' ~ 'All three products')) %>%
mutate(product = as_factor(product)) %>%
mutate(product = fct_relevel(product, "No nodes or preprints", "Only private storage", "osf4m", "OSF Node", "Public OSF node storage", "Registrations", "Preprints", "Supplemental preprint node", "Public Sharing & Registrations", "Public Sharing & Preprints", "Registrations & Preprints", "All three products", "Total")) %>%
arrange(product) %>%
mutate_if(~is.numeric(.), list(`prc` = ~round(100*./.[13],1))) %>%
filter(product != 'Total') %>%
gt() %>%
tab_header(
title = 'Users by Product Type'
) %>%
cols_merge(columns = vars(eartharxiv_preprints, eartharxiv_preprints_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(engrxiv_preprints, engrxiv_preprints_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(inarxiv_preprints, inarxiv_preprints_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(psyarxiv_preprints, psyarxiv_preprints_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(socarxiv_preprints, socarxiv_preprints_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(osf_preprints, osf_preprints_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(thesiscommons_preprints, thesiscommons_preprints_prc), pattern = '{1} ({2}%)') %>%
cols_label(product = 'Product Type',
     eartharxiv_preprints = 'Eartharxiv',
     engrxiv_preprints = 'Engrxiv',
     inarxiv_preprints = 'Inarxiv',
     psyarxiv_preprints = 'Psyarxiv',
     socarxiv_preprints = 'Socarxiv',
     osf_preprints = 'OSFPps',
     thesiscommons_preprints = 'ThesisComs',) %>%
tab_footnote(
footnote = 'Only includes nodes that are not osf4m or supplemental nodes created during the preprint process',
locations = cells_data(
  columns = vars(product),
...
By this definition, of all the active users are the site there are:

```{r echo = FALSE, warning=FALSE}
user_categorization %>% distinct(id, .keep_all = TRUE) %>% filter(is_active == TRUE) %>% group_by(depth_user) %>% tally()
```

The table below shows creation rates by product type, with depth users showing high levels of engagement with all product types.
...
cols_merge(columns =vars(depth, depth_prc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(nondepth, nondepth_prc), pattern = '{1} ({2}%)') %>%
cols_label(product = 'Product Type',
     depth = 'Depth Users',
...
## Disciplines of depth users

Total depth users who have created a preprint: `r nrow(user_categorization %>% distinct(id, .keep_all = TRUE) %>% filter(is_active == TRUE,preprint_creator == 1,depth_user == 'depth'))`


```{r}
# are depth users from particular fields, using preprint provider as proxy for discipline
user_categorization %>%
  distinct(id, .keep_all = TRUE) %>%
  filter(is_active == TRUE,
         preprint_creator == 1,
         depth_user == 'depth') %>%
  select(id, contains('creator')) %>%
  left_join(user_preprint_data %>% filter(type == 'created'), by = 'id') %>%
  group_by(provider) %>%
  summarize(osf4m = sum(osf4m_creator == 1),
            preprint= sum(preprint_creator == 1),
            suppnode = sum(suppnode_creator == 1),
            nodes = sum(osf_node_creator),
            publicstorage = sum(public_sharing_creator == 1),
  ...
  filter(preprint >= 25) %>%
  melt(id.vars = 'provider', variable.name = 'product') %>% 
  dcast(product ~ provider) %>%
  mutate(ina_perc = round(100 * inarxiv/(.[2, 'inarxiv']),2),
         osf_perc = round(100 * osf/(.[2, 'osf']),2),
         psy_perc = round(100 * psyarxiv/(.[2, 'psyarxiv']),2),
         soc_perc = round(100 * socarxiv/(.[2, 'socarxiv']),2),
         thesis_perc = round(100 * thesiscommons/(.[2, 'thesiscommons']),2)) %>%
  mutate(product = case_when(product == 'nodes' ~ 'OSF Node',
                     product == 'osf4m' ~ 'osf4m',
                     product == 'preprint' ~ 'Preprints',
                     product == 'publicstorage' ~ 'Public OSF node storage',
                     ...
                     product == 'allthree' ~ 'All three products')) %>%
    mutate(product = as_factor(product)) %>%
    mutate(product = fct_relevel(product, "osf4m", "OSF Node", "Public OSF node storage", "Registrations", "Preprints", "Supplemental preprint node", "Public Sharing & Registrations", "Public Sharing & Preprints", "Registrations & Preprints", "All three products")) %>%
    arrange(product) %>%
gt() %>%
tab_header(
title = 'Creation - Product Type by Preprint Provider'
) %>%
cols_merge(columns =vars(inarxiv, ina_perc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(osf, osf_perc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(psyarxiv, psy_perc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(socarxiv, soc_perc), pattern = '{1} ({2}%)') %>%
cols_merge(columns = vars(thesiscommons, thesis_perc), pattern = '{1} ({2}%)') %>%
cols_label(product = 'Product Type',
     inarxiv = 'INA-Rxiv',
     osf = 'OSFPps',
     psyarxiv = 'PsyArXiv',
...
## When are depth users from?

Are depth users created during specific times? Similar to 'all three products' users, depth users tend to have been created a couple of years ago, so it takes a while to work up to 'depth usage' based on the current definition. 

```{r echo=FALSE, warning=FALSE}
 graph <- ggplot(user_categorization %>% distinct(id, .keep_all = TRUE) %>%  filter(is_active == 'TRUE' & depth_user == 'depth'), aes(by_day_confirmed)) + 
  geom_freqpoly(binwidth = 30) +
  xlab('When user was confirmed') +
  ylab('Number of users') +
  labs(title = 'When were depth users confirmed?') +
  scale_x_date(breaks = date_breaks("3 month"), labels = date_format("%b %y"))
...
`r round(nrow(no_product_users)/sum(user_categorization$is_active == TRUE),2)`% are not contributors or creators of any non-deleted/withdrawn content on the OSF. What do we know about these users?
...
no_product_users %>%
  ...
  kable_styling(bootstrap_options = c("striped", "hover"))
```
#### Are they doing actions then taking themselves off products?

Only `r round(100*nrow(no_product_users %>% filter(added_contributor > 0 | added_file > 0 | wiki_edited > 0 | updated_file > 0))/nrow(no_product_users),2)`% have done this, based on `contributor_added`, `file_updated`, `file_added`, and/or `wiki_updated` logs on non-deleted/withdrawn content. 

Other possibilities for what they might be doing include only creating/contritbuting on content that was later deleted, or signign up for an account because they thought they needed to to view content. The high proportion of users with preprint and registries tags that haven't created a preprint or registration might also suggest that users think they need to sign up for an account to do something. 


# Potential Next Steps:

* Are their discipline differences? (infered from OSF4M and preprint services until we have better disciplinary information)
* Eventually we'll beging to get user_tags on invited users, so we'll be able to include them in the user origin descriptives. 
* Once file timelines are fixed, sharing behavior can be added to the action flow diagrams.
