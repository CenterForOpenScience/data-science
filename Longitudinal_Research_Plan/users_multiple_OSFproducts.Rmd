...
arrange(product) %>%
mutate(meetings_prc = round(100*meetings/nrow(user_categorization %>% filter(is_active == TRUE & is_invited == FALSE & user_origin == 'meetings')),2),
 osf_prc = round(100*OSF/nrow(user_categorization %>% filter(is_active == TRUE & is_invited == FALSE & user_origin == 'OSF')),2),
 registries_prc = round(100*registries/nrow(user_categorization %>% filter(is_active == TRUE & is_invited == FALSE & user_origin == 'registries')),2),
 preprints_prc = round(100*preprints/nrow(user_categorization %>% filter(is_active == TRUE & is_invited == FALSE & user_origin == 'preprints')),2)) %>%
gt() %>%
tab_header(
title = 'Users by Product Type'
) %>%
