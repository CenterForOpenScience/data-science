...
transpose_storage %>% 
  ...
  tab_footnote(
    footnote = 'No deleted files [even ones > 30 days old] counted starting Sept 2020',
    locations = cells_data(
      columns = vars(Type),
      rows = c(4, 8)
    )
  ...
  tab_footnote(
    footnote = 'After consult with product, no deleted files [even ones > 30 days old] included in total starting Sept 2020',
    locations = cells_data(
      columns = vars(Type),
      rows = c(1, 5)
    )
...
* Number of public nodes at/above cap: `r nrow(by_node_storage %>% 
        filter(target_is_public == TRUE & gig_tot_size >= 50))`%
* Percentage of public nodes at/above cap: `r round(100 * nrow(by_node_storage %>% 
        filter(target_is_public == TRUE & gig_tot_size >= 50))/nrow(by_node_storage %>% 
        filter(target_is_public == TRUE)),2)`%

\

* Number of private nodes at/above cap: `r nrow(by_node_storage %>% 
        filter(target_is_public == FALSE & gig_tot_size >= 5))`%
* Percentage of private nodes at/above cap: `r round(100 * nrow(by_node_storage %>% 
        filter(target_is_public == FALSE & gig_tot_size >= 5))/nrow(by_node_storage %>% 
        filter(target_is_public == FALSE)),2)`%

\

* Percentage of Top-level projects with at least 1 node that exceed the storage cap: 
    + public: `r round(100 * (by_node_storage %>% 
        ...
        filter(num_nodes > 1) %>%
        nrow()),2)`%
    + private: `r round(100 * (by_node_storage %>% 
