...
SELECT name,
		COALESCE(COUNT(DISTINCT (CASE WHEN type = 'osf.node' THEN root_id END)),0) AS num_topleve_projects,
		COALESCE(COUNT(DISTINCT (CASE WHEN type = 'osf.registration' THEN root_id END)),0) AS num_topleve_reg,
		COALESCE(COUNT(DISTINCT (CASE WHEN type = 'osf.node' THEN node_id END)) AS num_nonreg_nodes,
		COALESCE(COUNT(DISTINCT (CASE WHEN type = 'osf.registration' THEN node_id END)),0) AS num_reg_nodes,
		COALESCE(COUNT(DISTINCT (CASE WHEN type = 'osf.node' THEN osf4I_files.file_id END)),0) AS num_nonreg_files,
		COALESCE(COUNT(DISTINCT (CASE WHEN type = 'osf.registration' THEN osf4I_files.file_id END)),0) AS num_reg_files,
