...
WITH existing_files AS (SELECT COUNT(*) AS num_files, target_object_id, MIN(created) AS first_osf_file_created, MAX(created) AS last_osf_file_created
	...
	node_categories AS (SELECT osf_abstractnode.id AS node_id, 
						   type, 
						   CASE WHEN is_fork IS FALSE THEN created ELSE forked_date END AS created, 
						   deleted_date, 
						   is_fork, 
						   is_deleted, 
						   is_public, 
...
SELECT osf_osfuser.id,
		...
		COALESCE(added_contributor,0) AS added_contributor,
		COALESCE(added_file,0) AS added_file,
		COALESCE(updated_file,0) AS updated_file,
		COALESCE(wiki_edited,0) AS wiki_edited,
		COALESCE(made_public,0) AS made_public,
		COALESCE(made_private,0) AS made_private,
		COALESCE(addon_added,0) AS addon_added,
		first_add_contrib,
		first_upload_file,
		first_update_file,
		first_wiki_edit,
