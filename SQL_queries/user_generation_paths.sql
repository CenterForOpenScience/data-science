...
/* query to add quarterly numbers*/

/* get all tag and SSO information*/
WITH user_tag_info AS (SELECT osf_osfuser.id AS user_id, 
	...
	/* count up, by month, new direct sign-ups by source provider that happened in the last quarter*/
	 new_signups AS (SELECT COUNT(user_id) AS new_signups, 
						...
						WHERE is_registered IS TRUE AND is_invited IS FALSE AND 
								tag_type = 'source' AND
								date_confirmed < date_trunc('month', now()) AND date_confirmed >= date_trunc('month', now() - interval '3 month')
	 ...
	 new_invites AS (SELECT COUNT(user_id) AS new_invitees, product, date_trunc('month', now() - interval '1 month')::DATE AS month
 						FROM user_tag_info
 						WHERE is_invited IS TRUE AND tag_type = 'source'
 						GROUP BY product),

	 /* count up, by month, new invited users who have claimed their account in the last quarter by source provider, based on when they claimed */
  	 new_claims AS (SELECT COUNT(user_id) AS new_claims,
	 				...
	 				WHERE is_invited IS TRUE AND date_confirmed IS NOT NULL AND
	 						date_confirmed < date_trunc('month', now()) AND date_confirmed >= date_trunc('month', now() - interval '3 month') AND 
	 						tag_type = 'claimed'
	 				GROUP BY product, date_trunc('month', date_confirmed)),

  	 /* cross-join all possible dates & tags to get date for all months */
  	 dates AS (SELECT month
					FROM generate_series(date_trunc('month', now() - interval '3 month')::DATE, 
							date_trunc('month', now() - interval '1 month')::DATE, '1 month'::interval) as month),
