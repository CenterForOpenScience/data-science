...
## Perc of type within timewindows
...
ggplotly(projects_by_product %>%
  filter(!is.na(products)) %>%
  mutate(month_date = floor_date(pc_by_day, 'month')) %>%
  group_by(products, month_date) %>%
  summarize(percent = n()) %>%
  group_by(month_date) %>%
  mutate(percent = percent/sum(percent)*100) %>%
  ggplot(aes(x = month_date, y = percent, color = products)) +
  geom_line() +
  scale_x_date(breaks = date_breaks("3 month"), labels = date_format("%b %y")) +
  theme(axis.text.x  = element_text(angle=45,hjust = 1,vjust = 1)))
...
* For content logs, I've grouped together some types of add-on logs for simplicity.
* Quartiles created from graphs above by number of projects
...
quartiles %>%
  mutate_at(vars(-log_type), ~round(.,2)) %>%
  kable() %>%
  kable_styling()
```
...
projects_and_logs %>%
  filter(is.na(is_fork) & !is.na(days_create_lastlog) & !is.na(products)) %>%
  select(log_type, days_create_lastlog, products) %>%
  mutate(days_create_lastlog = round(log2(ceiling(as.numeric(days_create_lastlog))))) %>%
  group_by(log_type, products, days_create_lastlog) %>%
  summarize(percent = n()) %>%
  group_by(log_type, days_create_lastlog) %>%
  mutate(percent = percent/sum(percent)*100) %>%
  filter(days_create_lastlog >= -22) %>%
  ggplot(aes(x = days_create_lastlog, y = percent, color = products)) +
  geom_line() +
  facet_wrap(~log_type) +
  labs(y = 'Percentage of projects by product', x = 'Rounded Log2 Days Between Last Log and Date Created')
...
log_graph <- ggplot(projects_and_logs %>% filter(is.na(is_fork) & !is.na(days_create_lastlog) & !is.na(products)),aes(days_create_lastlog, color = as.factor(year))) + 
  geom_freqpoly(binwidth = 25) +
  facet_grid(products ~ log_type) +
...
ggplotly(projects_and_logs %>%
  filter(is.na(is_fork) & !is.na(days_create_lastlog) & !is.na(products)) %>%
  select(year, log_type, days_create_lastlog, products) %>%
  mutate(days_create_lastlog = round(log2(ceiling(as.numeric(days_create_lastlog))))) %>%
  group_by(year, log_type, products, days_create_lastlog) %>%
  summarize(percent = n()) %>%
  group_by(year, log_type, days_create_lastlog) %>%
  mutate(percent = percent/sum(percent)*100) %>%
  filter(days_create_lastlog >= -22, year >= 2014) %>%
  ggplot(aes(x = days_create_lastlog, y = percent, color = products)) +
  geom_line() +
  geom_point() +
...
## Log type by project type & quartile

\

**Caveats**

* I've excluded log types that show up less than 5% for all quartiles and all project types for simplicity
...
# find projects by product by quartile in content logs
total_ns_content <- common_logs %>% 
                filter(log_type == 'content') %>% 
                distinct(root_id, .keep_all = T) %>% 
                group_by(products, quartile) %>% 
                tally()

# find projects by product by quartile in content logs
total_ns_metadata <- common_logs %>% 
                filter(log_type == 'metadata') %>% 
                distinct(root_id, .keep_all = T) %>% 
                group_by(products, quartile) %>% 
                tally()
...
# setup functions for creating table subsections
table_input_content <- function(product_type) {
  common_logs %>%
  filter(log_type == 'content') %>%
  group_by(content_log_cats, root_id) %>%
  slice(1L) %>%
  ungroup() %>%
  group_by(products, quartile, content_log_cats) %>%
  tally() %>%
  ungroup() %>%
  left_join(total_ns_content, by = c('products', 'quartile')) %>%
  mutate(percentages = round(100*n.x/n.y, 2)) %>%
  filter(!is.na(products)) %>%
  select(-c(n.x, n.y)) %>%
  filter(products == (!! product_type)) %>%
  pivot_wider(names_from = c(quartile), values_from = percentages)
}

table_input_metadata <- function(product_type) {
  common_logs %>%
  filter(log_type == 'metadata') %>%
  group_by(action, root_id) %>%
  slice(1L) %>%
  ungroup() %>%
  group_by(products, quartile, action) %>%
  tally() %>%
  ungroup() %>%
  left_join(total_ns_metadata, by = c('products', 'quartile')) %>%
  mutate(percentages = round(100*n.x/n.y, 2)) %>%
  filter(!is.na(products)) %>%
  select(-c(n.x, n.y)) %>%
  filter(products == (!! product_type)) %>%
...
# table for content logs
bind_rows(table_input_content('Empty_Private'),
      table_input_content('Files_Private'),
      table_input_content('Files_Public'),
      table_input_content('Reg_Private'),
      table_input_content('Files_Reg_Public')) %>%
mutate_all(~replace(., is.na(.), 0)) %>%
bind_rows(total_ns_content %>% 
        pivot_wider(names_from = quartile, values_from = n) %>% 
        filter(!is.na(products)) %>% 
        mutate(content_log_cats = 'Total N') %>% 
        mutate_if(is.integer, as.double) %>%
        select(products, content_log_cats, First, Fourth, Second, Third)) %>% 
  select(products, content_log_cats, First, Fourth, Second, Third) %>%  
  group_by(products) %>%
  gt() %>%
  cols_move_to_end(
  ...
  tab_header(title = 'Percentages by Quartile',
                 subtitle = 'For content logs') %>%
  cols_align(align = c('center'), columns = T) %>%
  data_color(
    columns = vars(First,
                  Second,
...
```{r warning=FALSE, echo=FALSE, message=FALSE}
# table for metadata logs
rbind(table_input_metadata('Empty_Private'),
      table_input_metadata('Files_Private'),
      table_input_metadata('Files_Public'),
      table_input_metadata('Reg_Private'),
      table_input_metadata('Files_Reg_Public')) %>%
mutate_all(~replace(., is.na(.), 0)) %>%
bind_rows(total_ns_metadata %>% 
      pivot_wider(names_from = quartile, values_from = n) %>% 
      filter(!is.na(products)) %>% 
      mutate(action = 'Total N') %>% 
      mutate_if(is.integer, as.double) %>%
      select(products, action, First, Fourth, Second, Third)) %>% 
select(products, action, First, Fourth, Second, Third) %>%  
group_by(products) %>%
  gt() %>%
  cols_move_to_end(
