...
storage_data <- read_csv(paste0(here::here('Data_Usage/'), 'osf_storage_metrics.csv'), col_types = cols(date = col_datetime()))[-60,] %>%
...
# download and read in monthly cap info
...
```{r public & private storage counts, warning=FALSE, include=FALSE}
...
```{r formatting storage data for tables, warning=FALSE, include=FALSE}
...
storage_dt <- storage_data %>%
                      ...
                      select(date, month, total_tib, pubnode_tib, privatenode_tib, deleted_tib, node_tib, preprint_tib, regs_tib) %>%                    
...
```{r overall storage table, warning=FALSE, echo=FALSE}
# overall storage
# cells_data() is deprecated, changed to cells_body()
transpose_storage %>% 
...
```{r monthly storage table, warning=FALSE, echo=FALSE}
# monthly storage
# cells_data() is deprecated, changed to cells_body()
transpose_storage %>% 
  filter(Type == 'Nodes' | Type == 'Registrations' | Type == 'Preprints') %>%
  gt() %>%
  cols_align(align = c('center')) %>%
  tab_header(
...
```{r formatting storage data for graphs, warning=FALSE, echo=FALSE}
...
data_graph$storage_type <- fct_recode(data_graph$storage_type, 
...
data_graph$storage_type <- fct_relevel(data_graph$storage_type, c('Public Nodes', 'Private Nodes', 'Preprints',  'Registrations', 'Deleted Content'))
```
...
```{r monthly storage graph, warning=FALSE, echo=FALSE}
...
graph  <- plot_ly(data_graph, x = ~date, y = ~ perc_of_total, color =  ~storage_type, colors='Spectral',
                ...
                layout(xaxis = list(title = "",
                                    # adjusted nticks (used to be nrow(storage_data))
                                    nticks = 30,
                                    tickformat = '%b %y'),
...
```{r calculate monthly numbers, warning=FALSE, echo=FALSE, message = FALSE}
...
# append these monthly numbers onto running sheet and upload back to osf
...
```{r storage cap graph, warning=FALSE, echo=FALSE}
...
```{r private nodes above cap table, warning=FALSE, echo=FALSE}
by_node_storage %>% 
        ungroup() %>%
        filter(target_is_public == FALSE & gig_tot_size >= 5) %>% 
...
```{r public nodes above cap table, warning=FALSE, echo=FALSE}
by_node_storage %>% 
        ungroup() %>%
        filter(target_is_public == TRUE & gig_tot_size >= 50) %>% 
        select(target_guid, gig_tot_size) %>% 
