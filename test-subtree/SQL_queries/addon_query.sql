...
/* count of the number of nodes that have each addon connected fully and overall number of nodes with at least one storage addon */

WITH addons_per_node AS (SELECT id, 
		COUNT(osf_abstractnode.id) filter (where bitbucket_repo IS NOT NULL) AS has_bitbucket,
		COUNT(osf_abstractnode.id) filter (where S3_folder IS NOT NULL) AS has_S3,
		COUNT(osf_abstractnode.id) filter (where box_folder IS NOT NULL) AS has_box,
		COUNT(osf_abstractnode.id) filter (where github_repo IS NOT NULL) AS has_github,
	...
	LEFT JOIN (SELECT owner_id, folder_id AS google_folder
					...
					WHERE is_deleted IS FALSE AND folder_id IS NOT NULL) AS google
	ON osf_abstractnode.id = google.owner_id
	WHERE osf_abstractnode.type = 'osf.node' AND osf_abstractnode.is_deleted IS FALSE AND (spam_status IS NULL OR spam_status = 4) AND title NOT LIKE 'Bookmarks'
	GROUP BY osf_abstractnode.id)


SELECT 
	COUNT(CASE WHEN has_bitbucket > 0 THEN id END) as bitbucket,
	COUNT(CASE WHEN has_S3 > 0 THEN id END) as S3,
	COUNT(CASE WHEN has_box > 0 THEN id END) as box,
	COUNT(CASE WHEN has_github > 0 THEN id END) as github,
	COUNT(CASE WHEN has_gitlab > 0 THEN id END) as gitlab,
	COUNT(CASE WHEN has_zotero > 0 THEN id END) as zotero,
	COUNT(CASE WHEN has_dropbox > 0 THEN id END) as dropbox,
	COUNT(CASE WHEN has_figshare > 0 THEN id END) as figshare,
	COUNT(CASE WHEN has_mendeley > 0 THEN id END) as mendeley,
	COUNT(CASE WHEN has_onedrive > 0 THEN id END) as onedrive,
	COUNT(CASE WHEN has_owncloud > 0 THEN id END) as owncloud,
	COUNT(CASE WHEN has_dataverse > 0 THEN id END) as dataverse,
	COUNT(CASE WHEN has_google > 0 THEN id END) as google,
	COUNT(CASE WHEN has_bitbucket > 0 OR has_S3 > 0 OR has_box > 0 OR has_github > 0 OR has_gitlab > 0 OR
					has_dropbox > 0 OR has_figshare > 0 OR
					has_onedrive > 0 OR has_owncloud > 0 OR has_dataverse > 0 OR has_google > 0 THEN id END) AS any_storage_addons
	FROM addons_per_node;
